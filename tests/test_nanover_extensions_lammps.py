...
def test_real_units_conversions() -> None:
    pos_to_nm, _force_factor, mass_to_amu = get_unit_conversions("real")
...
def test_metal_units_conversions() -> None:
    _pos_to_nm, _force_factor, _mass_to_amu = get_unit_conversions("metal")
    assert _force_factor == pytest.approx(1.03643e-3)
